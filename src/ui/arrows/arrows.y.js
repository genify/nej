/*
 * ------------------------------------------
 * 垂直箭头控件实现文件
 * @version  1.0
 * @author   genify(caijf@corp.netease.com)
 * ------------------------------------------
 */
var f = function(){
    // variable declaration
    var e = NEJ.P('nej.e'),
        p = NEJ.P('nej.ui'),
        __proArrowsY,
        __supArrowsY;
    if (!!p._$$ArrowsY) return;
    // ui css text
    var _seed_css = e._$pushCSSText('\
                     .#<uispace>{width:1.5em;height:4em;overflow:hidden;$<transition>-property:$<transform>;$<transition>-duration:200ms;$<transition>-timing-function:ease-out;background-size:1.5em 4em;background:url(data:image/png;base64,) center center no-repeat;}\
                     .#<uispace>{$<transform>:$<rotate|a=180deg>;}\
                     .#<uispace>-reverse{$<transform>:$<rotate|a=360deg>;}');
    /**
     * 垂直箭头控件
     * @class   {nej.ui._$$ArrowsY} 垂直箭头控件
     * @extemds {nej.ui._$$Arrows}
     * @param   {Object} 可选配置参数，已处理参数列表如下：
     */
    p._$$ArrowsY = NEJ.C();
    __proArrowsY = p._$$ArrowsY._$extend(p._$$Arrows);
    __supArrowsY = p._$$ArrowsY._$supro;
    /**
     * 初始化外观信息
     * @protected
     * @method {__initXGui}
     * @return {Void}
     */
    __proArrowsY.__initXGui = function(){
        this.__seed_css = _seed_css;
    };
};
NEJ.define('{lib}ui/arrows/arrows.y.js',['{lib}ui/arrows/arrows.js'],f);