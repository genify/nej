...
var f = function(){
     ...
     * @api    {Array.prototype.reverse}
     * @return {Array} 数组
     */
    if (!_pro.reverse){
        _pro.reverse = function(){
            // TODO performance sparse array
            for(var i=0,j=this.length-1,tmpi,tmpj;i<j;i++,j--){
                tmpi = this[i];
                tmpj = this[j];
     ...
     * @api    {Array.prototype.lastIndexOf}
     * @param  {Variable} 要搜索的元素
     ...
     * @api    {Array.prototype.reduceRight}
     * @param  {Function} 处理函数
     * @param  {Variable} 初始值
     * @return {Variable} 计算结果
     */
...
NEJ.define('{lib}patched/trident/es.js',
          ['{lib}patched/es.js'],f);